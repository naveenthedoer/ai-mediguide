...
"""Imports documents from GCS into a Agent Platform Search data store.
...
class _DiscoveryEngineService:
    """Minimal Discovery Engine v1alpha REST client."""
    def __init__(self, endpoint: str, credentials, project_id: str):
    ...
    def get_connector(self, name: str) -> dict:
        url = f"{self._endpoint}/v1alpha/{name}"
        resp = requests.get(url, headers=self._headers())
        resp.raise_for_status()
        return resp.json()

    def import_documents(self, parent: str, body: dict) -> dict:
        url = f"{self._endpoint}/v1alpha/{parent}:import"
...
def _build_service(location: str, project_id: str) -> _DiscoveryEngineService:
    """Build a minimal Discovery Engine v1alpha REST client."""
    credentials, _ = google.auth.default()
    if location == "global":
        endpoint = "https://discoveryengine.googleapis.com"
    else:
        endpoint = f"https://{location}-discoveryengine.googleapis.com"
    return _DiscoveryEngineService(endpoint, credentials, project_id)


@click.command()
@click.option("--project", required=True, help="GCP project ID.")
@click.option("--region", required=True, help="GCP region/location.")
@click.option(
    "--collection-id", required=True, help="Collection ID for the data connector."
)
...
def main(project: str, region: str, collection_id: str, wait: bool) -> None:
    ...
    try:
        connector = service.get_connector(connector_name)
    ...
    # Trigger the import
    try:
        operation = service.import_documents(
            branch,
            {
                "gcsSource": {
                    "inputUris": gcs_uris,
    ...
    for _attempt in range(1, 61):
        time.sleep(10)
        try:
            status = service.get_operation(lro_name)
        except Exception as e:
